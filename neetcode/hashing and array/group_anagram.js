...
var groupAnagrams2 = function(strs) {
    ...
    for(let i=0; i<strs.length; i++){
        const arrValue = strs[i];
        let mapKey = strs[i].split("").sort((a,b)=>a.localeCompare(b)).join("");
        const mapValue = mp.get(mapKey)
        let arr = []
        if(mapValue){
            mp.set(mapKey,[...arr, ...mapValue, arrValue])
        }else{    
            mp.set(mapKey,[...arr, arrValue])
        }
